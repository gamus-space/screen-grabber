...
if (process.argv.length < 5) {
	console.error("usage: crop.js [width]x[height] file out_file");
	process.exit(1);
}
...
(async () => {
	...
	if (inFile.endsWith('.webp') || outFile.endsWith('.webp')) {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'crop-'));
	}
	let tempFile;
	...
	console.log(
		...
		`${size[0] ?? image.bitmap.width}x${size[1] ?? image.bitmap.height}`
	);
	image.crop(
		size[0] ? (image.bitmap.width - size[0])/2 : 0,
		size[1] ? (image.bitmap.height - size[1])/2 : 0,
		size[0] || image.bitmap.width,
		size[1] || image.bitmap.height,
	).write(tempFile ?? outFile);
	if (tempFile) {
